...
function playOnMouse() {
	if (event.target.innerHTML == 'Play Demo!') {
		audioSample();
	}
...
function audioSample () {
	for (let i = 0; i < songNotes.length; i++){
			let k = notes[songNotes[i]];
			setTimeout(function(){
				document.getElementById(k.id).style.backgroundColor = '#bdbebd';
				const sound = new Audio(k.src);
				sound.play();
			}, ms * (i + 1));
...
document.addEventListener('mouseup', () => {
	if (typeof notes[keyNote] !== 'undefined') {
	document.getElementById(keyNote).style.backgroundColor = event.target.style.color;
	}
});
